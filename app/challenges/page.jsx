...
const CATEGORIES = [
  { id: 'all',         label: 'All',         symbol: '◈', bg: '#EDE9E6', pat: null,                                                                                                                                                                        text: '#2C2825' },
  ...
  { id: 'color',       label: 'Color',       symbol: '●', bg: '#F5E8C4', pat: null,                                                                                                                                                                        text: '#5C3A00' },
  ...
  { id: 'mood',        label: 'Mood',        symbol: '◐', bg: '#C8D4F5', pat: 'linear-gradient(rgba(48,76,137,.15) 1px,transparent 1px),linear-gradient(90deg,rgba(48,76,137,.15) 1px,transparent 1px)', patSize: '8px 8px',                               text: '#1E2C5C' },
];
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        <span style={{ fontSize: 9, color: '#9C7060', letterSpacing: '.05em', textTransform: 'uppercase' }}>
          <Highlighted color="rgba(221,183,114,.45)">progress</Highlighted>
        </span>
        <span style={{ fontSize: 11, color: '#2C2825', fontWeight: 700 }}>{done} / {total} this week</span>
      </div>
      <div style={{ height: 12, background: 'rgba(196,168,130,.2)', border: '3px solid #2C2825', borderRadius: '8px 14px 10px 6px / 12px 6px 14px 8px', overflow: 'hidden', position: 'relative' }}>
        ...
        <main className="pt-[100px] pb-20 px-8 max-w-5xl mx-auto">
          ...
          <div style={{ background: 'rgba(237,233,230,.97)', borderRadius: '16px 24px 18px 14px / 22px 14px 24px 18px', padding: '40px 48px 52px', boxShadow: '6px 6px 0 rgba(44,40,37,.2)' }}>
            ...
            <div className="flex items-end justify-between mb-8">
              ...
              {/* XP chip */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: '3px solid #2C2825', borderRadius: '12px 18px 14px 10px / 16px 10px 18px 12px', padding: '6px 14px', background: '#F5E8C4', boxShadow: '4px 4px 0 rgba(44,40,37,.52)', transform: 'rotate(0.6deg)' }}>
                <span style={{ fontSize: 18, color: '#ddb772' }}>✦</span>
                <div>
            ...
            <div className="flex gap-4 mb-8 items-stretch">
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                ...
                <div style={{ backgroundColor: '#FFF0EB', backgroundImage: 'repeating-linear-gradient(transparent 0px,transparent 17px,rgba(200,100,80,.42) 17px,rgba(200,100,80,.42) 18px)', border: '3px solid #2C2825', borderRadius: '14px 22px 16px 12px / 20px 12px 22px 16px', padding: '28px 24px 20px', transform: 'rotate(-0.4deg)', boxShadow: '5px 5px 0 rgba(44,40,37,.6)', height: '100%', boxSizing: 'border-box' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 14 }}>
                    {TODAY.tags.map(t => (
                      <span key={t} style={{ fontSize: 11, padding: '2px 10px', border: '2px solid #C4A882', borderRadius: '8px 14px 10px 6px / 12px 6px 14px 8px', color: '#9C7060' }}>{t}</span>
                    ))}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                    <span style={{ fontSize: 11, color: '#B4AEA8' }}>{TODAY.duration}</span>
                    <Link href={TODAY.href} style={{ background: '#C84B3A', color: '#FFF8F0', padding: '8px 22px', fontFamily: 'Mansalva', fontSize: 16, textDecoration: 'none', border: '2px solid #2C2825', borderRadius: '10px 16px 12px 8px / 14px 8px 16px 10px', boxShadow: '3px 3px 0 rgba(44,40,37,.52)' }}>
                      start →
                    </Link>
                  </div>
                </div>
              ...
              {/* Weekly theme — golden wheat, referencing #ddb772 */}
              <div style={{ width: 210, position: 'relative', flexShrink: 0 }}>
                ...
                <div style={{ backgroundColor: '#F5E8C4', backgroundImage: 'repeating-linear-gradient(rgba(180,140,60,.25) 0,rgba(180,140,60,.25) 1px,transparent 1px,transparent 10px),repeating-linear-gradient(90deg,rgba(180,140,60,.25) 0,rgba(180,140,60,.25) 1px,transparent 1px,transparent 10px)', backgroundSize: '10px 10px', border: '3px solid #2C2825', borderRadius: '12px 20px 14px 10px / 18px 10px 20px 14px', padding: '28px 18px 20px', transform: 'rotate(0.6deg)', boxShadow: '5px 5px 0 rgba(44,40,37,.55)', height: '100%', boxSizing: 'border-box' }}>
                  <span style={{ fontSize: 9, letterSpacing: '.05em', textTransform: 'uppercase', color: '#9C7A30' }}>
                    <Highlighted color="rgba(221,183,114,.5)">{WEEKLY_THEME.subtitle}</Highlighted>
                  </span>
                  <h3 style={{ fontFamily: 'Mansalva', fontSize: 18, color: '#5C3A00', lineHeight: 1.2, marginTop: 8 }}>{WEEKLY_THEME.title}</h3>
                  <p style={{ fontSize: 11, color: '#7A5A20', marginTop: 8, lineHeight: 1.55 }}>{WEEKLY_THEME.description}</p>
                  <p style={{ fontSize: 10, color: '#9C7A30', marginTop: 14 }}>Ends in <strong>{WEEKLY_THEME.daysLeft}</strong> days</p>
                </div>
              </div>
            </div>
                ...
                {CATEGORIES.map((cat, idx) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: activeCategory === cat.id ? '3px 3px 0 rgba(200,75,58,.65)' : '3px 3px 0 rgba(44,40,37,.45)',
                      transition: 'all .18s',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 32 }}>
              {UPCOMING.map((c, idx) => {
                const card = (
                  <div style={{ backgroundColor: c.done ? '#F5EDE0' : '#EDE9E6', backgroundImage: c.done ? 'repeating-linear-gradient(transparent 0px,transparent 15px,rgba(200,100,80,.3) 15px,rgba(200,100,80,.3) 16px)' : undefined, border: '3px solid #2C2825', borderRadius: '12px 18px 14px 10px / 16px 10px 18px 12px', padding: '16px', transform: `rotate(${UPCOMING_ROT[idx]}deg)`, boxShadow: '4px 4px 0 rgba(44,40,37,.52)', opacity: c.href || c.done ? 1 : 0.62, minHeight: 150, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
                    {c.isNew && !c.done && (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        {c.tags.map(t => (
                          <span key={t} style={{ fontSize: 10, padding: '2px 8px', border: '2px solid #C4A882', borderRadius: '6px 10px 8px 5px / 8px 5px 10px 6px', color: '#9C7060' }}>{t}</span>
                        ))}
                      </div>
            ...
            {/* Continuity tracker */}
            <div style={{ backgroundColor: '#EDE9E6', backgroundImage: 'radial-gradient(circle,rgba(44,40,37,.08) 1.5px,transparent 1.5px)', backgroundSize: '14px 14px', border: '3px solid #2C2825', borderRadius: '14px 20px 16px 12px / 18px 12px 20px 16px', padding: '24px', marginBottom: 24, boxShadow: '5px 5px 0 rgba(44,40,37,.52)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', border: '3px solid #2C2825', borderRadius: '10px 16px 12px 8px / 14px 8px 16px 10px', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between' }}>
                  {STREAK.map((d, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 40, height: 40, border: `3px solid ${d.today ? '#C84B3A' : '#2C2825'}`, borderRadius: '8px 12px 10px 6px / 10px 6px 12px 8px', background: d.done ? '#C84B3A' : d.today ? 'rgba(200,75,58,.08)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: d.done ? '#FFF8F0' : '#C4A882', fontSize: 16 }}>
                        {d.done ? '✓' : ''}
                      </div>
                  ...
                  {WEEKS.map((week, wi) => (
                    <div key={wi} style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 4, flex: 1 }}>
                        {week.map((done, di) => (
                          <div key={di} style={{ flex: 1, height: 24, border: '2px solid #2C2825', borderRadius: '4px 8px 6px 3px / 6px 3px 8px 4px', background: done ? '#C84B3A' : 'transparent', opacity: wi === 0 && di > 2 ? 0.22 : 1 }} />
                        ))}
                      </div>
                    </div>
                  ))}
            ...
            {/* Rewards */}
            <div style={{ border: '3px solid #2C2825', borderRadius: '14px 20px 16px 12px / 18px 12px 20px 16px', padding: '20px 24px', marginBottom: 24, background: '#EDE9E6', boxShadow: '5px 5px 0 rgba(44,40,37,.45)' }}>
              ...
              <div style={{ display: 'flex', gap: 28 }}>
                {REWARDS.map((r, i) => (
                  <div key={r.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, opacity: r.earned ? 1 : 0.42 }}>
                    <div style={{ width: 54, height: 54, border: `3px ${r.earned ? 'solid' : 'dashed'} #2C2825`, borderRadius: '50%', background: r.earned ? '#ddb772' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: r.earned ? '#2C2825' : '#C4A882', boxShadow: r.earned ? '3px 3px 0 rgba(44,40,37,.45)' : 'none', transform: `rotate(${BADGE_ROT[i]}deg)` }}>
                      {r.symbol}
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <button style={{ backgroundColor: '#5C3A1E', backgroundImage: 'radial-gradient(circle,rgba(255,255,255,.28) 1.5px,transparent 1.5px)', backgroundSize: '8px 8px', border: '3px solid #2C2825', borderRadius: '12px 20px 14px 10px / 18px 10px 20px 14px', padding: '24px', transform: 'rotate(-0.4deg)', boxShadow: '5px 5px 0 rgba(44,40,37,.6)', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
                <span style={{ fontSize: 28, color: '#ddb772', display: 'block' }}>+</span>
              ...
              <button style={{ backgroundColor: '#FFF0EB', backgroundImage: 'repeating-linear-gradient(transparent 0px,transparent 15px,rgba(200,100,80,.45) 15px,rgba(200,100,80,.45) 16px)', border: '3px solid #2C2825', borderRadius: '12px 20px 14px 10px / 18px 10px 20px 14px', padding: '24px', transform: 'rotate(0.5deg)', boxShadow: '5px 5px 0 rgba(44,40,37,.5)', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
